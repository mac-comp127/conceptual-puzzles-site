class Cohort < ApplicationRecord
  ...
  scope :active, -> do
    where('start_date <= ? and end_date >= ?', official_current_time, official_current_time)
  end
  ...
  def self.official_current_time
    @official_current_time ||= time_zone.now
  end

  def start_time
    start_date.in_time_zone(Cohort.time_zone)
  end

  def end_time
    end_date.in_time_zone(Cohort.time_zone) + 1.day  # ends at midnight the _next_ day, not midnight on the end date
  end

  def started?
    Cohort.official_current_time >= start_time
  end

  def ended?
    Cohort.official_current_time > end_time
  end
